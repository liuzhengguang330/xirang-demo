...
class CRMP: 
    #def __init__(self, tau, gain_mat,t0,del_t, qp0, bhp0,J):
    ...
    def bhp_term(self):
        ...
        #sum_bhp = ((sum_bhp)/np.exp(1/tau)) + bhp_t/tau
...
#        include_press =  self.include_press
    ...
    def compute_grads(self,q_obs):
        ...
        for i in range(N_inj):
            dmse_dlambda.append(2*np.sum((q_obs[1:,:] - q_pred[1:,:])*-Grad_Lambda[:,i,:],axis=0)/np.max(q_obs,axis=0)**2) 
...
#        dmse_dq0 = 2*np.sum(((q_obs - q_pred)/(q_obs+0.001))*-Grad_Q0,axis=0)/np.max(q_obs,axis=0)**2
    ...
    def compute_loss(self,q_obs):   
        q_pred = self.q_pred        
        mse = np.sum((((q_obs - q_pred)**2)),axis=0)/np.max(q_obs,axis=0)**2#/np.sum((q_obs - q_pred)**2,axis=0)        
        return mse
    ...
    def obj_func_fit(self,x,input_series,q_obs):
        ...
        self.qp0 = x[N_prd+(N_prd*N_inj):(2*N_prd)+(N_prd*N_inj)].reshape(1, N_prd)
        if self.include_press:
            self.J = x[(2*N_prd)+(N_prd*N_inj):].reshape(1, N_prd)
    ...
    def fit_model(self,input_series, q_obs, init_guess):
        ...
        self.tau = tau_fit
        self.gain_mat = gain_mat_fit
        self.qp0 = q0_fit.reshape(1, N_prd)
        if self.include_press:
            J_fit = fit_param[(2*N_prd)+(N_prd*N_inj):]
            param_fits.append(J_fit)
            self.J = J_fit.reshape(1, N_prd)
